...
def feedForward(X_scaled):
    sum1 = np.dot(X_scaled, weights_0) + bias_0
    sum1 = np.array(sum1)
    layer_1 = sigmoid(sum1)  # Activation function.
    sum2 = np.dot(layer_1, weights_1) + bias_1
    layer_2 = sigmoid(sum2)  # Activation function.
    return layer_1, layer_2

def backPropogate(weights_0, bias_0, weights_1, bias_1):
    ...
    bias_1 -= np.sum(layer_2_delta, axis=0, keepdims=True) * learning_rate
    bias_0 -= np.sum(layer_1_delta, axis=0, keepdims=True) * learning_rate
    return weights_0, bias_0, weights_1, bias_1
...
weights_0 = np.array([[0.10473281, 0.23991864, 0.51106061, 0.97739018],
                        [0.46591006, 0.54318817, 0.58782883, 0.68117129],
                        [0.0502301, 0.22142866, 0.86126238, 0.72482657]])
bias_0 = np.array([[0.15025418, 0.73481849, 0.90219478, 0.30605943]])
weights_1 = np.array([[0.62996657], [0.25984049], [0.72180012], [0.81730325]])
bias_1 = np.array([[0.09842751]])
...
for i in range(EPOCHS):
    ...
    weights_0, bias_0, weights_1, bias_1 = \
        backPropogate(weights_0, bias_0, weights_1, bias_1)
