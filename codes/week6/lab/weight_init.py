...
class FluDiagnoser(nn.Module):
    def __init__(self, n_inputs, n_outputs, n_neurons=8,
                 weight_init=init.xavier_uniform_,
                 weight_constraint=1.0):
        super().__init__()
        self.layer = nn.Linear(n_inputs, n_neurons)
        self.act = nn.ReLU()
    ...
    def forward(self, x):
        ...
        return self.prob(self.output(
            self.act(self.layer(x))
        ))
